...
## Шаг 4: Настройка Custom Start Command (опционально)

**⚠️ ВАЖНО:** Оставьте поле **Custom Start Command** пустым!

Railway автоматически использует команду из `railway.json`:
```
...
3. Если поле пустое - это правильно! Railway использует команду из `railway.json`

**Если нужно указать явно** (не рекомендуется):
```
java -Xmx512m -Xms256m -jar app.jar
```

**Подробнее:** См. `RAILWAY_START_COMMAND.md`

---

## Шаг 5: Настройка Persistent Volume (ВАЖНО!)
